...
def test_build_classes(mock_db_connection, reset_db):
    ...
    """
    output1: list[vc.Vehicle] = ws.build_classes(
        'src/sql/select_all_vehicles.sql',
        'all_vehicles',
        )

    output2: list[vc.Vehicle] = ws.build_classes(
        'src/sql/select_tax_due_vehicles.sql',
        'tax_due',
        )

    output3: list[vc.Vehicle] = ws.build_classes(
        'src/sql/select_service_due_vehicles.sql',
        'service_due',
    )

    output4: list[vc.Vehicle] = ws.build_classes(
        'BG70LKM',
        'num_plate',
        )
